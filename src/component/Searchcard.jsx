    ...
    return (
        <div className="w-[90%] lg:h-56 md:h-48 rounded-md flex md:flex-row flex-col lg:gap-8 md:gap-3 gap-4 items-center ">
            ...
            <div className="flex flex-col md:w-[65%] w-full lg:h-52 md:h-full lg:gap-3 md:gap-2">
                ...
                <div className="text-zinc-600 xl:text-[17px] lg:text-[15px] text-[17px]  font-medium">
                    <span>{parseInt(views).toLocaleString()} views</span> &nbsp;<span>&#9679; {time}</span>
                </div>
                <Link to={`/Channel/${channelId}`} className="text-zinc-600 xl:text-[17px] lg:text-[15px] md:text-[16px] text-[18px] font-medium">
                    {channel_name}
                </Link>
                <p className="dot text-zinc-600 xl:text-[17px] lg:text-[16px] md:text-[17px] text-[18px]">
                    {description}
                </p>
            </div>
        </div>
